...
export default function App() {
  const [view, setView] = useState<'home' | 'categories' | 'products' | 'cart' | 'admin'>(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      if (params.get('cart')) return 'cart';
    } catch {}
    return 'home';
  });
  ...
  const [cart, setCart] = useState<CartItem[]>(() => {
    // Check if there's a shared cart in the URL
    try {
      const params = new URLSearchParams(window.location.search);
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="bg-white/5 p-4 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border border-white/5">
                    <p className="text-xs sm:text-sm text-white/50 mb-3 sm:mb-4 font-medium tracking-wide uppercase">Կիսվել Զամբյուղով</p>
                    <div className="grid grid-cols-3 gap-2 sm:gap-3">
                      <button
                        onClick={shareViaViber}
                        className="flex flex-col items-center justify-center gap-1.5 sm:gap-2 py-3 sm:py-4 px-2 rounded-2xl border border-white/10 hover:border-[#7360F2]/50 hover:bg-[#7360F2]/10 active:scale-95 transition-all group"
                      >
                        <svg viewBox="0 0 24 24" className="w-6 h-6 sm:w-8 sm:h-8 fill-[#7360F2] group-hover:scale-110 transition-transform" xmlns="http://www.w3.org/2000/svg">
                          <path d="M11.4 0C5.5.1.8 4.9.8 10.8c0 2.8 1 5.4 2.8 7.4l-1.8 5.4 5.6-1.8c1.9 1.1 4 1.7 6.3 1.7h.1C19.6 23.5 24 18.8 24 13c0-6.2-5.6-13-12.6-13zm5.9 16.4c-.3.9-1.7 1.7-2.4 1.8-.6.1-1.4.1-2.2-.1-.5-.1-1.1-.4-1.9-.7-3.3-1.4-5.4-4.7-5.6-4.9-.2-.2-1.5-2-.1-3.6.2-.2.4-.5.7-.7.6-.5 1.2-.3 1.6-.1.4.2.7.7 1 1.2.3.5.6 1 .5 1.4-.1.4-.4.6-.7.9-.3.3-.6.5-.5.9.1.4.5 1.2 1.2 1.9.8.9 1.5 1.3 2 1.5.4.2.7.2.9-.1.2-.2.6-.7.9-1 .3-.3.6-.3 1-.1.4.2 2.3 1.1 2.7 1.3.4.2.7.3.8.5 0 .4-.2 1.5-.5 2.3-.2.7-.2.8 0 .8z"/>
                        </svg>
                        <span className="text-[9px] sm:text-[11px] font-bold text-white/60 group-hover:text-[#7360F2] transition-colors">VIBER</span>
                      </button>
                      <button
                        onClick={shareViaWhatsApp}
                        className="flex flex-col items-center justify-center gap-1.5 sm:gap-2 py-3 sm:py-4 px-2 rounded-2xl border border-white/10 hover:border-[#25D366]/50 hover:bg-[#25D366]/10 active:scale-95 transition-all group"
                      >
                        <svg viewBox="0 0 24 24" className="w-6 h-6 sm:w-8 sm:h-8 fill-[#25D366] group-hover:scale-110 transition-transform" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span className="text-[9px] sm:text-[11px] font-bold text-white/60 group-hover:text-[#25D366] transition-colors">WHATSAPP</span>
                      </button>
                      <button
                        onClick={shareViaTelegram}
                        className="flex flex-col items-center justify-center gap-1.5 sm:gap-2 py-3 sm:py-4 px-2 rounded-2xl border border-white/10 hover:border-[#2AABEE]/50 hover:bg-[#2AABEE]/10 active:scale-95 transition-all group"
                      >
                        <svg viewBox="0 0 24 24" className="w-6 h-6 sm:w-8 sm:h-8 fill-[#2AABEE] group-hover:scale-110 transition-transform" xmlns="http://www.w3.org/2000/svg">
                          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
                        </svg>
                        <span className="text-[9px] sm:text-[11px] font-bold text-white/60 group-hover:text-[#2AABEE] transition-colors">TELEGRAM</span>
                      </button>
                    </div>
                  </div>
