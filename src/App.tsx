                ...
                <div className="space-y-4 sm:space-y-6">
                  {cart.map(item => (
                    <div key={item.id} className="flex gap-3 sm:gap-4 bg-white/5 p-3 sm:p-4 rounded-2xl border border-white/5">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white/5 rounded-xl overflow-hidden relative flex-shrink-0">
                        <img 
                            ...
                            {order.items.map(item => (
                              <div key={item.id} className="flex gap-3 bg-black/30 p-3 rounded-xl border border-white/5">
                                <div className="w-12 h-12 bg-white/5 rounded-lg overflow-hidden">
                                  <img 
                                    src={optimizeImageUrl(item.image, 100)} 
                                    className="w-full h-full object-cover" 
                                    referrerPolicy="no-referrer" 
                                    loading="lazy"
                                    crossOrigin="anonymous"
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-orange-400">{item.price_at_time?.toLocaleString() || item.price?.toLocaleString()} ֏</p>
                                </div>
                              </div>
                            ))}
                          </div>
...
function CategoryActionCard({ title, desc, color, onClick }: { title: string, desc: string, color: string, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`${color} p-5 sm:p-8 md:p-10 rounded-[2rem] sm:rounded-[2.5rem] text-left flex flex-col justify-between min-h-[180px] sm:min-h-[240px] md:min-h-[300px] hover:scale-[1.02] transition-transform shadow-2xl group w-full`}
    >
      <div>
      ...
      <div className="mt-4 sm:mt-6 md:mt-8">
        <span className="bg-gradient-to-r from-blue-500/40 to-orange-500/40 backdrop-blur-md px-4 py-1.5 sm:px-6 sm:py-2 rounded-full font-bold text-[10px] sm:text-xs md:text-sm group-hover:from-blue-500/60 group-hover:to-orange-500/60 transition-all">
          Դիտել →
        </span>
      </div>
    </button>
...
function CategoryCard({ title, image, onClick }: { title: string, image: string, onClick: () => void }) {
  ...
  return (
    <motion.button 
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="relative h-56 sm:h-80 md:h-[350px] lg:h-[450px] rounded-[2rem] overflow-hidden group bg-zinc-900 w-full"
    >
      <img 
        src={optimizeImageUrl(image, 1000)} 
...
function ProductCard({ product, onAdd, priority }: { product: Product, onAdd: () => void, priority?: boolean }) {
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="bg-gradient-to-b from-white/10 to-white/[0.02] rounded-2xl sm:rounded-3xl border border-white/10 overflow-hidden flex flex-col hover:border-blue-500/30 transition-colors group"
    >
      <div className="aspect-square overflow-hidden relative bg-zinc-900">
        ...
        <img 
          src={optimizeImageUrl(product.image, priority ? 600 : 400)} 
          alt={product.name} 
          onLoad={() => setIsLoaded(true)}
          loading={priority ? "eager" : "lazy"}
          className={`w-full h-full object-cover transition-all duration-500 group-hover:scale-110 ${priority || isLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`} 
          referrerPolicy="no-referrer" 
          crossOrigin="anonymous"
        />
        <div className="absolute top-2 left-2 sm:top-4 sm:left-4 bg-black/60 backdrop-blur-md px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[8px] sm:text-[10px] font-bold tracking-widest uppercase border border-white/10">
          {product.code}
        </div>
      </div>
      <div className="p-3 sm:p-5 flex-1 flex flex-col">
        ...
        {product.min_quantity && product.min_quantity > 1 && (
          <p className="text-[8px] sm:text-[10px] text-orange-400/80 font-bold mb-3 sm:mb-4 bg-orange-400/10 w-fit px-1.5 py-0.5 rounded-md">ՄԻՆ. ՔԱՆԱԿ: {product.min_quantity}</p>
        )}
        <div className="mt-auto flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2">
          ...
          <button 
            onClick={onAdd}
            className="w-full xs:w-auto px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-br from-blue-600 to-orange-500 rounded-xl sm:rounded-2xl hover:shadow-lg hover:shadow-orange-500/20 transition-all active:scale-90 flex items-center justify-center"
          >
